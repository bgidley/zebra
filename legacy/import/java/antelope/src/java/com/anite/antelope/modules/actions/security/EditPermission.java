 ...
 *  
 */
public class EditPermission extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		if (form.isAllValid()) {
			...
			Field newPermissionNameField = (Field) form.getFields().get(
					"permissionname");
			...
			DynamicPermission permission = (DynamicPermission) permissionManager
					.getPermissionById(Long.valueOf(permissionIdField
							.getValue()));
			permissionManager.renamePermission(permission,
					newPermissionNameField.getValue());
		}
	}
}