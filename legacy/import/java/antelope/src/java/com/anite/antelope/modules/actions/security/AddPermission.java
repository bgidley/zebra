 ...
 *  
 */
public class AddPermission extends SecureAction {
    ...
    public void doPerform(RunData data, Context context) throws Exception {
        ...
        if (form.isAllValid()) {
            ...
            permissionField = (Field) fieldMap.get("permissionname");

            // check the name hasnt already been taken before adding it
            if (!permissionManager.checkExists(permissionManager
                    .getPermissionInstance(permissionField.getValue()))) {
                Permission permission;
                permission = permissionManager
                        .getPermissionInstance(permissionField.getValue());
                permissionManager.addPermission(permission);
            } else {
                permissionField.addMessage("\"" + permissionField.getValue()
