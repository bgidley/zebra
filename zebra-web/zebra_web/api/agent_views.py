...
from zebra_web.api import agent_engine

logger = logging.getLogger(__name__)


def ensure_agent():
    """Ensure the Zebra agent is initialized."""
    async_to_sync(agent_engine.ensure_initialized)()
...
def agent_dashboard(request):
    ...
    workflows = async_to_sync(library.list_workflows)()
    all_stats = async_to_sync(metrics.get_all_stats)()
    recent_runs = async_to_sync(metrics.get_recent_runs)(limit=10)
    ...
    return render(request, "pages/agent_dashboard.html", context)
...
def workflow_detail(request, workflow_name):
    ...
    try:
        ...
        stats = async_to_sync(metrics.get_stats)(workflow_name)
        workflows = async_to_sync(library.list_workflows)()
...
def workflow_create(request):
    ...
    try:
        ...
        if request.headers.get("HX-Request"):
            ...
            response["HX-Redirect"] = f"/agent/workflows/{workflow_name}/"
            return response
...
def run_goal_execute(request):
    ...
    try:
        result = async_to_sync(agent_loop.process_goal)(goal)
...
def run_detail(request, run_id):
    ...
    run = async_to_sync(metrics.get_run)(run_id)
    if not run:
...
def run_rate(request, run_id):
    ...
    try:
        rating = int(rating)
        async_to_sync(agent_loop.record_rating)(run_id, rating)
...
def recent_runs(request):
    ...
    runs = async_to_sync(metrics.get_recent_runs)(limit=limit)
