...
- [x] 6.4 Verify deploy is gated on smoke; failed smoke blocks deploy — confirmed: oke_deploy is skipped whenever oke_smoke fails (pipeline #223 showed this)
- [x] 6.5 Full pipeline green on the in-cluster runner — all six jobs (lint/unit/e2e/oke_build/oke_smoke/oke_deploy) green on master pipeline #231 (sha d8cc05d). Fixed during F111: (1) tags missing on OKE jobs; (2) short-name image pull rejection; (3) runner registration args rejected with glrt tokens; (4) claude image wrongly pinned to web git SHA (#112); (5) CI RBAC moved out-of-band (#113); (6) oke_smoke port-forward timeout → use ClusterIP DNS; (7) ORA-01940 on schema cleanup → kill sessions first
